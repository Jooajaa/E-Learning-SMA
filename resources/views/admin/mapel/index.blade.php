                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
                    <div>
                        ...
                        <p class="text-indigo-100 mt-3 max-w-2xl">
                            Kelola daftar mata pelajaran yang digunakan oleh guru dan siswa dalam LMS SMA.
                        </p>
                    </div>

                    <div class="bg-white/15 border border-white/20 rounded-2xl p-5 min-w-[180px]">
                        <p class="text-indigo-100 text-sm">Total Mapel</p>
                        <p class="text-3xl font-extrabold mt-1">{{ $mapel->count() }}</p>
                    </div>
                </div>
            </div>
                                ...
                                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 hover:shadow-md transition">
                                    ...
                                    <div class="grid grid-cols-2 gap-3 text-sm mb-5">
                                        <div class="bg-slate-50 rounded-xl p-3 border border-slate-200">
                                            ...
                                            <p class="font-bold text-slate-800 mt-1">{{ $item->kode_mapel ?? '-' }}</p>
                                        </div>

                                        <div class="bg-slate-50 rounded-xl p-3 border border-slate-200">
                                            <p class="text-slate-500">SKS / Jam</p>
                                            <p class="font-bold text-slate-800 mt-1">{{ $item->sks ?? '-' }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <details class="mb-3">
                                        ...
                                        <form action="{{ route('admin.mapel.update', $item->id) }}" method="POST" class="mt-4 space-y-3">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <input type="text" name="kode_mapel" value="{{ $item->kode_mapel }}"
                                                class="w-full rounded-xl border-slate-300 shadow-sm">

                                            <input type="number" name="sks" value="{{ $item->sks }}"
                                                class="w-full rounded-xl border-slate-300 shadow-sm">
